...
public final class KafkaProducerSingleton {
	...
	public void init(String topic, int retry) {
		...
		if (null == kafkaProducer) {
			...
			//properties.put("zookeeper.connect", "192.168.51.196:2181");// 声明zk
			//properties.put("serializer.class", StringEncoder.class.getName());
			properties.put("bootstrap.servers", "192.168.51.196:9092");// 声明kafka broker
	...
	public void sendKafkaMessage(final String message) {
		...
		ProducerRecord<String, String> record = new ProducerRecord<String, String>(topic,0, keys.get(0),message);
		...
		kafkaProducer.send(record, new Callback() {
			public void onCompletion(RecordMetadata recordMetadata, Exception exception) {
				if (null != exception) {
					...
					log.info("Send success topic:{}\t key:{}\t patition:{}\t message:{}",topic,"",nextInt,message);
				}
			}
		});
	}
