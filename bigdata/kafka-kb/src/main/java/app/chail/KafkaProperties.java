...
public final class KafkaProperties {
    ...
    private KafkaProperties() {
        ...
        try {
            File proFile = new File(filePath + "conf/producer.properties");

            if (proFile.exists()) {
                producerProps.load(new FileInputStream(filePath + "conf/producer.properties"));
            }
            File conFile = new File(filePath + "conf/producer.properties");
            if (conFile.exists()) {
                consumerProps.load(new FileInputStream(filePath + "conf/consumer.properties"));
            }
            File serFile = new File(filePath + "conf/server.properties");
            if (serFile.exists()) {
                serverProps.load(new FileInputStream(filePath + "conf/server.properties"));
            }
            File cliFile = new File(filePath + "conf/client.properties");
            if (cliFile.exists()) {
                clientProps.load(new FileInputStream(filePath + "conf/client.properties"));
            }
        } catch (IOException e) {
