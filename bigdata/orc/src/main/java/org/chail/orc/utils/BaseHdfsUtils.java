...
import org.chail.orc.utils.krb.KerberosLoginSubject;
...
public class BaseHdfsUtils  {
     ...
     * @param log
     */
    public void printLog(HdfsFileType type, LogChannelInterface log){
        long var = System.currentTimeMillis();
        if(++lineNum%50000==0||var -lastPrintLogTime>printLogTime){
            StringBuffer sbf=new StringBuffer("HDFS-");
            sbf.append(type.getValue());
            sbf.append(" 获取行数 ");
            sbf.append(lineNum);
            lastPrintLogTime=var ;
            log.logBasic(sbf.toString());
        }
    }
